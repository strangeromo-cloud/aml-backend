...
function kicker(slide, text) { slide.addText(text, { x: 0.6, y: 0.5, w: 9, h: 0.3, fontFace: FONT, fontSize: 12, bold: true, color: TEAL, charSpacing: 3 }); }
function title(slide, text) { slide.addText(text, { x: 0.6, y: 0.82, w: 12.1, h: 0.9, fontFace: FONT, fontSize: 30, bold: true, color: INK }); }

(async () => {
  const ic = {};
  const need = { bolt: Fa.FaBolt, layer: Fa.FaLayerGroup, search: Fa.FaSearch, robot: Fa.FaRobot, chart: Fa.FaChartLine, sync: Fa.FaSyncAlt, gavel: Fa.FaGavel, brain: Fa.FaBrain, scale: Fa.FaBalanceScale, eye: Fa.FaEye, arrow: Fa.FaArrowRight, check: Fa.FaCheckCircle, lock: Fa.FaLock };
  ...
  s.addText("AML", { x: 0.9, y: 1.6, w: 2.4, h: 1.1, fontFace: FONT, fontSize: 40, bold: true, color: NAVY, align: "center", valign: "middle", fill: { color: TEAL }, rectRadius: 0.2 });
  s.addText("付款风险识别系统", { x: 0.9, y: 2.95, w: 11, h: 1.0, fontFace: FONT, fontSize: 46, bold: true, color: WHITE });
  s.addText("从「整体评分」到「分级告警 + 自我进化」", { x: 0.92, y: 4.05, w: 11, h: 0.6, fontFace: FONT, fontSize: 20, color: "AECDD6" });
  s.addText("面向法务团队的系统讲解", { x: 0.92, y: 5.6, w: 11, h: 0.5, fontFace: FONT, fontSize: 14, color: "8FB2BD" });
  ...
  stats.forEach((st, i) => {
    ...
    s.addText(st.l, { x: x + 0.22, y: 3.18, w: 3.4, h: 0.6, fontFace: FONT, fontSize: 14, bold: true, color: INK });
  ...
  s.addText([
    ...
    { text: "目标改为：让法务每天能审的那几十笔，含金量最高。", options: { color: TEAL2, bold: true } },
  ], { x: 0.9, y: 5.12, w: 11.5, h: 0.9, fontFace: FONT, fontSize: 15, lineSpacingMultiple: 1.3 });
  pageNum(s, 2);
  ...
  s.addText("痛点", { x: 0.95, y: 4.3, w: 4.5, h: 0.3, fontFace: FONT, fontSize: 12, bold: true, color: RED });
  s.addText("强信号被「平均」稀释：一笔命中 2 条 30% 命中率硬规则的付款，被其它正常维度一平均就沉底、可能没人看。", { x: 0.97, y: 4.65, w: 5.0, h: 1.6, fontFace: FONT, fontSize: 13, color: INK, lineSpacingMultiple: 1.3 });
  ...
  entry.forEach(e => {
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: e.x, y: 1.95, w: 5.95, h: 1.55, fill: { color: PANEL }, line: { color: LINE, width: 1 }, rectRadius: 0.12, shadow: sh() });
    ...
    s.addText(e.t, { x: e.x + 1.05, y: 2.18, w: 4.7, h: 0.6, fontFace: FONT, fontSize: 16, bold: true, color: INK, valign: "middle" });
    s.addText(e.d, { x: e.x + 0.32, y: 2.85, w: 5.4, h: 0.55, fontFace: FONT, fontSize: 12.5, color: MUTED, lineSpacingMultiple: 1.1 });
  });
  s.addText("两入口合并成法务的一份统一告警清单", { x: 0.6, y: 3.6, w: 12, h: 0.35, fontFace: FONT, fontSize: 12.5, italic: true, color: TEAL, align: "center" });
  // closed loop strip
  ...
  loop.forEach((b, i) => {
    ...
    s.addShape(pres.shapes.OVAL, { x: x + 0.22, y: 4.45, w: 0.5, h: 0.5, fill: { color: PANEL }, line: { color: TEAL, width: 1.3 } });
    s.addImage({ data: b[1], x: x + 0.32, y: 4.55, w: 0.3, h: 0.3 });
    s.addText(b[0], { x: x + 0.85, y: 4.2, w: 1.8, h: 1.0, fontFace: FONT, fontSize: 14, bold: true, color: INK, valign: "middle" });
  ...
  s.addText("从「静态规则」走向「会进化的助手」", { x: 0.6, y: 1.0, w: 12, h: 0.9, fontFace: FONT, fontSize: 30, bold: true, color: WHITE });
  const innov = [
  ...
  innov.forEach((b, i) => {
    ...
    s.addText(b[1], { x: x + 2.5, y: 2.65, w: 1.1, h: 0.7, fontFace: FONT, fontSize: 34, bold: true, color: "29516A", align: "right" });
    s.addText(b[2], { x: x + 0.35, y: 3.9, w: 3.2, h: 0.5, fontFace: FONT, fontSize: 18, bold: true, color: WHITE });
    s.addText(b[3], { x: x + 0.37, y: 4.45, w: 3.25, h: 1.5, fontFace: FONT, fontSize: 12.5, color: "AECDD6", lineSpacingMultiple: 1.25 });
  });
  pageNum(s, 5);
  ...
  pipe.forEach((b, i) => {
    ...
    s.addText(b[1], { x: x + 0.12, y: 3.92, w: 2.46, h: 0.5, fontFace: FONT, fontSize: 11.5, color: MUTED, align: "center" });
  ...
  s.addText("把高价值告警顶上来，让系统越用越准", { x: 0.6, y: 1.15, w: 12, h: 1.0, fontFace: FONT, fontSize: 30, bold: true, color: WHITE });
  s.addText("分级告警替代整体评分（强信号不再被稀释），双入口覆盖单笔与跨笔手法，AI 调查提效，自我进化发现新风险 —— 所有分级变更都在「统计校准 + 人工审批」的可审计闸门内。", { x: 0.62, y: 2.55, w: 11.5, h: 1.3, fontFace: FONT, fontSize: 15, color: "DDEBEF", lineSpacingMultiple: 1.35 });
  s.addText("对法务团队", { x: 0.6, y: 3.95, w: 8, h: 0.35, fontFace: FONT, fontSize: 13, bold: true, color: TEAL2 });
  const vals = ["一份排好序的清单，不再大海捞针", "每笔都附「为什么」，便于解释", "可解释 · 可审计，经得起监管追问", "你的每次复核都在让系统更准"];
  vals.forEach((v, i) => {
    const x = 0.6 + (i % 2) * 6.15;
    const y = 4.35 + Math.floor(i / 2) * 0.62;
    s.addImage({ data: ic.check, x, y: y + 0.02, w: 0.3, h: 0.3 });
    s.addText(v, { x: x + 0.42, y: y - 0.05, w: 5.6, h: 0.45, fontFace: FONT, fontSize: 12.5, color: "DDEBEF", valign: "middle" });
  });
  s.addText("后续对接：真实数据源（企查查/天眼查/Dow Jones）· 部署 Hermes Agent 框架 · 接入付款系统", { x: 0.6, y: 5.95, w: 12, h: 0.4, fontFace: FONT, fontSize: 12, color: "8FB2BD" });
  ...
  console.log("WROTE lite deck (7 slides)");
})();
