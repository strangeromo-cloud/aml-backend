...
- `vendorScoreOf` = Σ(因子强度 × 权重)，`weights.vendor` = Criminals 40 / FATF 15 / CPI 15 / Offshore List 15 / Adverse Media 10 / State-owned 5（**必须合计 100**）。FATF/CPI/Offshore 实时读 `PUBLIC_SOURCES`；每条 vendor 规则都要在 `VENDOR_FACTOR_RULE` 里对应一个因子，否则规则命中却不进分数。
...
- **信号台账**：`signalHitMap` / `buildSignalLedger` 的"信号"= 14 条 `RULES`（`t('ruleNames')`），每条显示其固定 tier；`structuralTier` 从 `RULES` 取 tier。候选（`discoveredSignals`）仍走 PPV 分档。
...
4. **校准的"当前等级"必须来自规则表的 Tier**（`currentChoiceOf` = `signalOverrides` 覆盖值 → 否则 `RULES` 里的 tier），**不能从复核样本数反推**。曾因用样本数推导，导致 5 条有 Tier 的规则在校准里显示成"观察"，与规则表不一致。
...
**等级阶梯 = `T1 > T2 > T3 > observe`（待观察：继续跟踪但不报警）。已无"评分因子"这一档** —— 需求表里 13 条规则条条都有 Tier，所以任何规则都是 Tier 级的。

判定按顺序（`buildCalibration`）：
① 样本 < 4 → `insufficient`（样本不足），**维持当前等级、不下结论**（不再提议降为"候选"）
② 样本 ≥ 4 且 Wilson 95% 下界 ≥ 8% → `validated`（证据支持），维持规则表 Tier；只有 Agent 发现的无 Tier 候选才按 PPV 分档"挣到"一个 Tier（`keepOrEarn`）
③ 样本 ≥ 4 且下界 < 8% → `demoted`（准确率不足），**下调一级**（`demoteOne`：T1→T2、T2→T3、**T3→observe**，observe 是底）

**先样本关、再 PPV 关**。"降级"是负面判决需足够证据；样本不足宁可维持现状不冤枉。管理员可在下拉框里直接改成 T1/T2/T3/待观察，审批后写入 `signalOverrides`（该覆盖值即下次校准的"当前等级"）。
⚠️ `signalOverrides` 目前只影响校准界面显示，**不改 `paymentTier`**（付款分级仍直接读 `RULES` 的 tier）。要让校准结论真正生效，需把 `paymentTier` 改读 `currentChoiceOf`。
